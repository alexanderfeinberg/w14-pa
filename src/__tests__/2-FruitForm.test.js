...
describe("(18 points) FruitForm", () => {
  test('(1 points) FruitForm renders a form with a class of "fruit-form"', () => {
    const fruitForm = shallow(<FruitForm fruits={fruits} />);
  ...
  test("(2 points) FruitForm has a controlled input for the name field", () => {
    ...
    fruitForm
      ...
      .simulate("change", { target: { value: "Blueberry" } });
    expect(fruitForm.find('input[name="name"]').props().value).toEqual(
      "Blueberry"
    );
  });
  ...
  test("(2 points) FruitForm has a controlled input for the sweetness field", () => {
    ...
    fruitForm
      .find('input[name="sweetness"]')
      .simulate("change", { target: { value: "7" } });
    expect(fruitForm.find('input[name="sweetness"]').props().value).toEqual(
      "7"
    );
  });
  ...
  test("(2 points) FruitForm has controlled inputs for the seeds radio inputs", () => {
    ...
    expect(fruitForm.find('input[type="radio"]').at(0).props().value).toEqual(
      "no"
    );
    expect(fruitForm.find('input[type="radio"]').at(1).props().value).toEqual(
      "yes"
    );
    expect(fruitForm.find('input[type="radio"]').at(0).props().checked).toEqual(
      false
    );
    expect(fruitForm.find('input[type="radio"]').at(1).props().checked).toEqual(
      true
    );
    fruitForm
  ...
  test('(1 points) FruitForm shows an error in "ul.errors" if name is less than 3 characters', () => {
    const fruitForm = mount(<FruitForm fruits={fruits} />);
  ...
  test('(1 points) FruitForm shows an error in "ul.errors" if name more than 20 characters', () => {
    ...
    fruitForm
      .find('input[name="name"]')
      .simulate("change", { target: { value: "This is 21 characters" } });
    expect(fruitForm.find("ul.errors").children()).toHaveLength(1);
  });
